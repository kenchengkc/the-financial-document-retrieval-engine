...
from fdre.evals.runner import evidence_recall_at_k
...
def build_retrieval_development_diagnostic(
    ...
    *,
    ...
    candidate_recall = evidence_recall_at_k(
        question,
        candidate_pool,
        k=len(candidate_pool),
    )
    final_recall = evidence_recall_at_k(
        question,
        final_candidates,
        k=final_k,
    )
...
def write_retrieval_development_diagnostics(
    ...
    *,
    ...
    summary = summarize_retrieval_development_diagnostics(diagnostics, final_k=final_k)
    payload = {
    ...
    lines = [
        ...
        "",
        "This report is development-only diagnostic evidence. It does not change or replace the published historical benchmark contract.",
        "",
        f"- Questions: **{summary['question_count']}**",
        f"- Answerable questions: **{summary['answerable_question_count']}**",
        f"- Unexpected ticker-scope cases: **{summary['unexpected_ticker_scope_count']}**",
        f"- Unexpected section-scope cases: **{summary['unexpected_section_scope_count']}**",
        f"- Candidate-pool misses: **{summary['candidate_miss_count']}**",
        f"- Candidate-to-final rerank losses at K={final_k}: **{summary['rerank_loss_count']}**",
    ...
    lines.extend(
        [
            ...
            "## Provider usage",
            "",
            f"- Embedding query calls represented: **{summary['provider_usage']['embedding_query_calls']}**",
            f"- Estimated embedding query tokens: **{summary['provider_usage']['estimated_embedding_tokens']}**",
            f"- Reranker calls represented: **{summary['provider_usage']['reranker_calls']}**",
            f"- Candidate documents sent to reranking: **{summary['provider_usage']['reranker_candidate_documents']}**",
            "",
            "Query-only scope fields are compared with benchmark labels only to expose false positive scope. Empty inference is not treated as an entity-resolution error when the query never names the labeled issuer or section.",
        ]
    )
...
def summarize_retrieval_development_diagnostics(
    ...
    *,
    ...
    return {
        ...
        "unexpected_ticker_scope_count": sum(bool(item.unexpected_tickers) for item in diagnostics),
        "unexpected_section_scope_count": sum(bool(item.unexpected_sections) for item in diagnostics),
        "candidate_miss_count": sum(item.candidate_miss is True for item in answerable),
        "rerank_loss_count": sum(item.rerank_loss is True for item in answerable),
        "mean_candidate_recall": _mean_optional(item.candidate_recall for item in answerable),
        "mean_final_recall": _mean_optional(item.final_recall for item in answerable),
        "table_questions": {
            "answerable_question_count": len(table_answerable),
            "candidate_miss_count": sum(item.candidate_miss is True for item in table_answerable),
            "rerank_loss_count": sum(item.rerank_loss is True for item in table_answerable),
            "mean_candidate_recall": _mean_optional(
                item.candidate_recall for item in table_answerable
            ),
            "mean_final_recall": _mean_optional(item.final_recall for item in table_answerable),
        },
        "stage_latency_ms": {
        ...
        "provider_usage": {
            ...
            "reranker_calls": sum(item.reranker_candidate_count > 0 for item in diagnostics),
