...
pinned independent complete-history interval assert the same ticker state for every day covered
by an already-materialized provisional interval?  Only full interval containment is promotable.
...
STATE_SUPPORT_SCHEMA_VERSION = "fdre-hu-state-support-v1"
...
def plan_state_support(
    ...
    for values in by_symbol.values():
        values.sort(key=lambda item: (item.effective_from, item.effective_to or date.max, item.lineage_id))
    ...
    for interval in ordered:
        ...
        if key in seen_keys:
            raise ValueError(f"duplicate provisional state row: {interval.row_kind}/{interval.row_id}")
        seen_keys.add(key)
        ...
        if len(containing) == 1:
            ...
            reason = (
                ...
                "materialized provisional interval"
            )
        ...
        decisions.append(
            StateSupportDecision(
                ...
                status=status,
                lineage_id=lineage.lineage_id if lineage is not None else None,
                lineage_effective_from=lineage.effective_from if lineage is not None else None,
...
def corroborated_source_hash(decision: StateSupportDecision, *, plan_id: str) -> str:
    if not decision.promotable or decision.lineage_id is None or decision.lineage_source_hash is None:
        raise ValueError("only fully supported decisions have corroborated provenance")
    return _digest(
        {
            "schema_version": STATE_SUPPORT_SCHEMA_VERSION,
            "plan_id": plan_id,
